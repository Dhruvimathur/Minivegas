...
<html lang="en">
  ...
  <h5 class="card-title text-right" style="color:white;">Your current balance is: 
  <?php
...
$str=$_SESSION["uid"];
  ...
  ?></h5>
    ...
    <<div class="card-body" style="margin-top:200px;">
            <h5 class="card-title text-center" style="color:white; font-weight:900; font-family:cursive; font-size:40px;">Place your Bet :</h5>
...
die("connection failed: ".$conn->connect_error);
   ...
   //$betty=$_SESSION["betupdt"];
   //echo $_SESSION["betupdt"];
$sql="select vmoney from casino_tbl where userid='$str'";

	 $result=$conn->query($sql);
	 $row=$result->fetch_assoc();
	 $money=$row["vmoney"];
	if($bet1>=50 && $bet1<=$money)
	{
	  $money=$money-$bet1;
	
	  

$sql="UPDATE casino_tbl SET vmoney='$money',userbet='$bet1' where userid='$str'";

 
if($conn->query($sql)===TRUE)
{
echo "<script>alert('Your bet is done.');</script>";
...
echo "<script>
	window.location.href='inthegame.html';
	</script>";
	exit;
}
else
{
	  echo "<script>alert('Bet should be greater than 50 and smaller than Virtual money.');</script>";
	  echo "<script>
